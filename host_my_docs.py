...
def parse():
    ...
    p.add_argument(
        "-H", "--host", default="%s:%s" % (cfg.host, cfg.port), help="Host to use."
    )
...
def post(host, metadata, zippath):
    ...
    got = requests.post(
        address, data=metadata, files={"archive": (filename, open(zippath, "rb"))}
    )
    return got
...
def delete(host, metadata, deleteall=False):
    address = _makeaddr(host)
    address += "?name=%s&version=%s" % (metadata["name"], metadata["version"])
    if deleteall:
